<div class="modal fade text-dark" id="rejectedEdit{{ $rejected->id }}">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('rejecteds.update', $rejected->id) }}" method="POST">
                {{ csrf_field() }}
                {{ @method_field('PUT') }}
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="select2-primary">
                                        <select class="select2 form-control-border" data-placeholder="Select a State" data-dropdown-css-class="select2-primary" style="width: 100%;" name="application_id" required>
                                            <option value="{{ $rejected->application->id }}">
                                                @if ($rejected->application->debtor())
                                                    {{ $rejected->application->id }}.-{{ $rejected->application->debtor()->lastName }} {{ $rejected->application->debtor()->secondLastName }} {{ $rejected->application->debtor()->thirdLastName }} {{ $rejected->application->debtor()->name }}
                                                @else
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="select2-primary">
                                        <select class="select2 form-control-border" data-placeholder="Select a State" data-dropdown-css-class="select2-primary" style="width: 100%;" name="user_id" required>
                                            <option value="{{ $rejected->user->id }}">{{ $rejected->user->name }}</option>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea id="my-textarea" class="form-control" name="description" rows="3">{{ $rejected->description }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
